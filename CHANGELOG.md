...
- **pencapaian-coverage-sonarqube:** Berhasil menembus target Clean Code Quality Gate SonarQube dengan statement coverage **80.43%** (16.667 dari 20.723 statements ter-cover), 100% test suites lulus (78 test suites, 466 tests passed) tanpa kegagalan.
- **eksklusi-sementara-modul-transaksi:** Mengecualikan direktori modul transaksi (`**/transaksi/**` meliputi `pages/home/transaksi/**`, `composables/transaksi/**`, `schemas/transaksi/**`) dari kalkulasi coverage di [`vitest.config.ts`](file:///c:/Users/USER/Documents/GitHub/tambora-frontend/vitest.config.ts) (`coverage.exclude`) dan [`sonar-project.properties`](file:///c:/Users/USER/Documents/GitHub/tambora-frontend/sonar-project.properties) (`sonar.coverage.exclusions`) karena modul transaksi masih dalam tahap pengembangan aktif / belum difinalisasi.
- **ekspansi-test-suite-komprehensif:**
  - **Composables & Bisnis**: Pengujian 100% coverage pada [`composables/useOperasiPembangkit.ts`](file:///c:/Users/USER/Documents/GitHub/tambora-frontend/composables/useOperasiPembangkit.ts) (535 stmts), interceptor API & error handling pada [`composables/useApi.ts`](file:///c:/Users/USER/Documents/GitHub/tambora-frontend/composables/useApi.ts) dan [`composables/useAuth.ts`](file:///c:/Users/USER/Documents/GitHub/tambora-frontend/composables/useAuth.ts).
  - **Komponen Inti & GIS**: Penambahan unit test untuk komponen peta GIS berbasis OpenLayers [`components/base/BaseMap.vue`](file:///c:/Users/USER/Documents/GitHub/tambora-frontend/components/base/BaseMap.vue) dengan OpenLayers mocks, `BaseToastContainer`, `BaseDashboardHeader`, `BaseChart`, `BaseLoadingIndicatorPLN`, serta interaksi sidebar & header navigasi pada `appLayoutComponents.test.ts`.
  - **Form Engine & Login**: Pengujian mendalam [`components/base/FormFieldRenderer.vue`](file:///c:/Users/USER/Documents/GitHub/tambora-frontend/components/base/FormFieldRenderer.vue) mencakup kontrol radio, switch, phone, color, coordinate-picker, currency, password, dan multi-select, serta slider captcha drag-and-drop & form login.
  - **Pinia Auth Store & RBAC**: Pengujian mendalam alur logout, sinkronisasi sesi lokal (`localStorage`), reset timer idle, broadcast multi-tab, resolusi hak akses peran (`can`, `hasScope`, `hasMenuAccess`), caching data profil, dan retry otomatis status 401 pada [`test/stores/auth_extra.test.ts`](file:///c:/Users/USER/Documents/GitHub/tambora-frontend/test/stores/auth_extra.test.ts).
  - **Halaman Master PLN & Konfigurasi**: Penambahan skenario interaksi CRUD lengkap (create modal, submit, edit drawer, async detail modal view, dialog konfirmasi hapus, pencarian, paginasi, dan ekspor) pada seluruh halaman master unit PLN (`uik.vue`, `unit-layanan.vue`, `up2d.vue`, `upk.vue`, `uiw-uid.vue`, `sentral.vue`, `user.vue`, `asset.vue`, `cabang.vue`, `driver.vue`, `machine-condition.vue`, `organization.vue`, `ranting.vue`, `regional.vue`, `system.vue`, `permission.vue`) dan konfigurasi aplikasi (`akses-grup.vue`, `akses-level.vue`, `menu.vue`).
  - **Route Guard Middleware**: Pengujian komprehensif middleware proteksi rute [`middleware/auth.global.ts`](file:///c:/Users/USER/Documents/GitHub/tambora-frontend/middleware/auth.global.ts) mencapai 100% coverage (pengecekan izin menu, fallback cold-start fetchUserMe, dan redirect rute terlarang).
- **remediasi-linting-dan-type-safety:**
  - Mengeliminasi seluruh unused imports pada berkas-berkas pengujian baru (`@typescript-eslint/no-unused-vars`).
  - Memperbaiki kompatibilitas tipe konfigurasi plugin Vite/Vue dan penanganan auto-import `import.meta.client` di Vitest environment.
  - Menyelaraskan seluruh type error TypeScript IDE hingga bersih (0 error, 0 warning pada `npm run lint`).

### Penyelarasan 22 Parameter Master Aset Mesin, Integrasi Detail API, Year Picker PrimeVue & Profil AppHeader

- **penyelarasan-parameter-crud-asset:** Menyelaraskan seluruh 22 parameter data aset mesin (`kode_mesin`, `nama_mesin`, `kode_spln`, `serial_number`, `power_plant_id`, `system_id`, `kondisi_mesin`, `kode_bahan_bakar`, `daya_terpasang`, `daya_mampu_netto`, `daya_mampu_pasok`, `kapasitas`, `jenis_tegangan`, `tegangan_hv`, `tegangan_lv`, `merk_mesin`, `tipe_mesin`, `merk_generator`, `nama_trafo`, `status_kepemilikan_mesin`, `status_kepemilikan_kwh`, `tahun_operasi`) agar terkirim bersih pada Create (`CreateAssetRequest`) dan Update (`UpdateAssetRequest`), form create default kosong/bersih, serta memperbaiki mapping `kapasitas` agar tidak tertimpa oleh `daya_terpasang` pada [`types/master.types.ts`](file:///c:/Users/andym/Documents/Project%20Vue/tambora-frontend/types/master.types.ts), [`schemas/master/asset.schema.ts`](file:///c:/Users/andym/Documents/Project%20Vue/tambora-frontend/schemas/master/asset.schema.ts), dan [`pages/home/master/asset.vue`](file:///c:/Users/andym/Documents/Project%20Vue/tambora-frontend/pages/home/master/asset.vue).
- **integrasi-api-detail-asset:** Menghubungkan tombol view detail aset mesin dengan endpoint backend `GET /api/v1/assets/{id}` menggunakan composable `useAsyncDetail<AssetItem>`, memisahkan state `detailLoading` pada [`composables/master/useAsset.ts`](file:///c:/Users/andym/Documents/Project%20Vue/tambora-frontend/composables/master/useAsset.ts) agar tabel data utama tidak mengalami flickering/reload saat modal detail dibuka, serta menambahkan unit test pendukung pada [`test/composables/master_phase3.test.ts`](file:///c:/Users/andym/Documents/Project%20Vue/tambora-frontend/test/composables/master_phase3.test.ts).
- **komponen-year-picker-primevue:** Menambahkan dukungan tipe input `'year'` pada formulir dinamis via [`components/base/FormFieldRenderer.vue`](file:///c:/Users/andym/Documents/Project%20Vue/tambora-frontend/components/base/FormFieldRenderer.vue) dan [`types/form.types.ts`](file:///c:/Users/andym/Documents/Project%20Vue/tambora-frontend/types/form.types.ts) menggunakan komponen PrimeVue `<DatePicker view="year" date-format="yy" />` dengan converter dua arah otomatis untuk angka tahun (integer), serta menerapkannya pada field `tahun_operasi` di form Master Aset Mesin.
- **standarisasi-terminologi-sentral:** Menghilangkan pleonasme kata _"Sentral Pembangkit"_ menjadi istilah baku _"Sentral"_ di seluruh antarmuka navigasi ([`config/navigation.ts`](file:///c:/Users/andym/Documents/Project%20Vue/tambora-frontend/config/navigation.ts)), modul Master Sentral ([`pages/home/master/sentral.vue`](file:///c:/Users/andym/Documents/Project%20Vue/tambora-frontend/pages/home/master/sentral.vue)), tabel transaksi pembangkitan, dan skema form terkait.
- **profil-pengguna-appheader:** Mengganti tampilan organisasi/role pada dropdown profil header aplikasi ([`components/base/AppHeader.vue`](file:///c:/Users/andym/Documents/Project%20Vue/tambora-frontend/components/base/AppHeader.vue)) menjadi alamat email pengguna aktif (`email`).

### Refactoring Duplikasi Kode 100% Modul Master & Konfigurasi, Remediasi SonarQube & Penguatan Type Safety

- **refactor-duplikasi-kode-master-dan-konfigurasi:** Refactor komprehensif pada seluruh 16 halaman Master Data dan 3 halaman Konfigurasi Aplikasi untuk mengeliminasi duplikasi kode dari 5.7% menjadi **2.69%** (memenuhi ambang batas toleransi SonarQube Clean As You Go < 3.0%):
  - Membuat composable terpadu [`composables/useCrudState.ts`](file:///c:/Users/andym/Documents/Project%20Vue/tambora-frontend/composables/useCrudState.ts) untuk mengotomasi state pencarian, paginasi (`currentPage`, `pageSize`, `paginateList`), form modal, dan dialog konfirmasi hapus.
  - Membuat komponen reusable [`components/base/BaseTableActions.vue`](file:///c:/Users/andym/Documents/Project%20Vue/tambora-frontend/components/base/BaseTableActions.vue) dengan tombol Lihat Detail, Ubah, Hapus yang seragam, tooltip, accessibility label, dan slot `#extra` untuk aksi kustom (seperti tombol Setujui pada Sentral).
  - Menyelaraskan 16 halaman Master (`organization.vue`, `uiw-uid.vue`, `regional.vue`, `ranting.vue`, `sentral.vue`, `cabang.vue`, `system.vue`, `permission.vue`, `up2d.vue`, `upk.vue`, `unit-layanan.vue`, `uik.vue`, `driver.vue`, `machine-condition.vue`, `asset.vue`, `user.vue`) dan 3 halaman Konfigurasi Aplikasi (`akses-grup.vue`, `akses-level.vue`, `menu.vue`).
- **remediasi-security-prng-sonar:** Menuntaskan seluruh 5 open issues SonarQube Security (CWE-330 / S2245: Insecure PRNG) dengan beralih dari `Math.random()` ke Web Crypto API (`crypto.getRandomValues()`) melalui utilitas baru [`utils/cryptoRandom.ts`](file:///c:/Users/andym/Documents/Project%20Vue/tambora-frontend/utils/cryptoRandom.ts) (`getSecureRandom` dan `getNextSequenceId`), diterapkan pada `FormFieldRenderer.vue`, `useAppToast.ts`, `SliderCaptcha.vue`, dan `deviceMeta.ts`.
- **remediasi-reliability-maintainability:** Memperbaiki 45 open reliability issues dan 29 maintainability issues SonarQube:
  - Mengganti pemanggilan `isNaN` global ke `Number.isNaN`, `parseInt` ke `Number.parseInt`, dan bitwise truncate `| 0` ke `Math.trunc`.
  - Mencegah ReDoS (Regular Expression Denial of Service) pada regex kode level di `akses-level.vue`.
  - Meningkatkan accessibility WCAG dengan atribut `id`, `name`, `aria-label`, dan `htmlFor` pada form controls.
  - Memperbaiki penanganan null check Promise di `stores/auth.ts`.
- **penyempurnaan-type-safety-crud-state:** Memperbaiki seluruh 8 masalah TypeScript IDE pada `useCrudState.ts` (generic array universal pada `paginateList`), signature `deleteFn` di `executeDelete`, dan binding ID string eksplisit pada aksi delete modul Master Organisasi, Ranting, Regional, Sentral, dan UIW/UID, serta unit test `test/composables/crudState.test.ts`.
- **peningkatan-coverage-unit-test-komprehensif:** Menambahkan unit test komprehensif pada seluruh layer non-transaksi hingga mencapai **216 tests passed across 31 test suites (100% pass)**:
  - **`schemas/master`**: Coverage melonjak ke **99.8%** (skema validasi seluruh 18 modul master data ter-cover penuh).
  - **`schemas/konfigurasi-aplikasi`**: Coverage mencapai **100%** (skema Akses Level, Akses Grup, dan Menu).
  - **`utils/`**: Coverage mencapai **90.67%** dengan pembuatan unit test baru untuk [`utils/formatDate.ts`](file:///c:/Users/andym/Documents/Project%20Vue/tambora-frontend/utils/formatDate.ts) (edge cases zona waktu, guard zero date Go, dan formatting tanggal).
  - **`stores/auth.ts`**: Coverage meningkat menjadi **59.41%** (90% functions) dengan pengujian mendalam fungsi parsing hak akses (`extractPermissionKey`, `extractScopeKey`, `fetchUserAccess` caching, dan pembersihan sesi lokal).
  - **`composables/`**: Pengujian branch coverage untuk error handling (status HTTP 400/404/500, fallback dual-method DELETE/POST, network retry) pada Master dan Konfigurasi Aplikasi.
...
- **cascading-ranting-regional-sentral:** Implementasi filter bertingkat (_cascading filter_) Ranting berdasarkan Regional yang dipilih melalui relasi cabang (`useCabang`: `cabang.kode_wilayah` -> `ranting.kode_cabang`), auto-reset field `kode_ranting` saat pengguna memilih atau mengganti Regional baru, placeholder kondisional adaptif (_"Pilih Regional Terlebih Dahulu"_, _"Pilih Ranting"_, atau _"Tidak Ada Ranting di Regional Ini"_), serta status `disabled` otomatis pada field Ranting jika Regional belum ditentukan pada [`pages/home/master/sentral.vue`](file:///c:/Users/andym/Documents/Project%20Vue/tambora-frontend/pages/home/master/sentral.vue) dan [`schemas/master/sentral.schema.ts`](file:///c:/Users/andym/Documents/Project%20Vue/tambora-frontend/schemas/master/sentral.schema.ts).
- **integrasi-combo-sistem:** Menghubungkan combobox UPK (`upk_id`) langsung ke endpoint `/api/v1/upk` via `useUpk()` dan Unit Layanan (`service_unit_ids` multiple) ke `/api/v1/unit-layanan` via `useUnitLayanan()`, serta menyelaraskan payload Create dan Update Master Sistem agar mengirimkan ID entitas dan format koordinat/deskripsi yang konsisten pada [`pages/home/master/system.vue`](file:///c:/Users/andym/Documents/Project%20Vue/tambora-frontend/pages/home/master/system.vue), [`schemas/master/system.schema.ts`](file:///c:/Users/andym/Documents/Project%20Vue/tambora-frontend/schemas/master/system.schema.ts), dan [`types/master.types.ts`](file:///c:/Users/andym/Documents/Project%20Vue/tambora-frontend/types/master.types.ts).
...
- **notifikasi-sesi-ditabrak-perangkat-lain:** Menambahkan notifikasi toast khusus saat terjadi sesi ganda/ditabrak (_concurrent session_): _"Sesi Anda telah berakhir atau akun sedang digunakan di perangkat/peramban lain. Silakan login kembali."_ tanpa memicu modal hitung mundur inaktivitas palsu.
- **penyelarasan-id-combobox-master:** Menyelaraskan seluruh combobox relasional pada modul Master Data (Cabang, Ranting, Sentral, Pengguna, Permission) agar mengirimkan ID entitas (UUID) dan bukan kode/nama, dengan tetap mempertahankan nama parameter API asli backend dan menjaga label tabel/detail tetap deskriptif (_human-readable_).
...
- **pembongkaran-data-pengguna:** Ekstraksi objek `detailRecord.value.user` pada [`pages/home/master/user.vue`](file:///c:/Users/andym/Documents/Project%20Vue/tambora-frontend/pages/home/master/user.vue) untuk field profil, Published ID, riwayat aktivitas, serta pemetaan otomatis field `akses_grup` pada form edit.
...
- **standarisasi-toast-error-api:** Sentralisasi notifikasi error API ke interceptor global `useApi` (Single Source of Truth) dan menghapus pemanggilan `toast.error` lokal di blok `catch` pada 11 halaman master dan konfigurasi.
- **isolasi-loading-tabel-dan-refresh-otomatis:** Menghapus manipulasi state `loading.value` tabel dari fungsi mutasi (`create`, `update`, `delete`) di seluruh composable Master dan Konfigurasi Aplikasi. Tabel di latar belakang tidak lagi berkedip (flicker) jika input gagal/invalid, dan data tabel tetap disegarkan secara otomatis via pemanggilan `fetch...()` saat mutasi berhasil.
- **penyempurnaan-transisi-modal-sukses:** Menambahkan jeda waktu (delay 150ms) sebelum memunculkan popup `BaseSuccessModal` setelah drawer form ditutup di seluruh modul master dan konfigurasi aplikasi, mencegah konflik unmount transisi drawer dengan popup sukses.
- **penghapusan-status-approval-cabang:** Menghapus kolom status approval, field input form, dan detail status approval dari modul Master Cabang ([`pages/home/master/cabang.vue`](file:///c:/Users/andym/Documents/Project%20Vue/tambora-frontend/pages/home/master/cabang.vue), [`schemas/master/cabang.schema.ts`](file:///c:/Users/andym/Documents/Project%20Vue/tambora-frontend/schemas/master/cabang.schema.ts)).
- **penghapusan-status-dan-approval-ranting:** Menghapus kolom status approval dan status ranting, field input form, serta detail status dari modul Master Ranting ([`pages/home/master/ranting.vue`](file:///c:/Users/andym/Documents/Project%20Vue/tambora-frontend/pages/home/master/ranting.vue), [`schemas/master/ranting.schema.ts`](file:///c:/Users/andym/Documents/Project%20Vue/tambora-frontend/schemas/master/ranting.schema.ts)), menyelaraskan struktur data Ranting secara identik dengan Cabang (hanya Cabang, Kode, dan Nama).
- **standarisasi-ssot-modal-detail:** Sentralisasi pengelolaan metadata modal detail (Published ID, Tanggal Pembuatan, Dibuat Oleh, dan Riwayat Aktivitas) secara terpusat (Single Source of Truth) ke dalam [`components/base/BaseDetailModal.vue`](file:///c:/Users/andym/Documents/Project%20Vue/tambora-frontend/components/base/BaseDetailModal.vue) via prop `:record="detailRecord"`. Menghapus lebih dari 300 baris duplikasi boilerplate (`formattedCreatedDate`, `activityLogs`, zero-date fallback) pada 16 modul Master Data dan 3 modul Konfigurasi Aplikasi dengan tetap mempertahankan field detail spesifik (`detailDataItems`) dan slot tambahan di tiap modul.
- **standarisasi-master-organisasi:** Memulihkan dan merapikan modul Master Organisasi dengan nama resmi dan bersih "Organisasi" (tanpa embel-embel Legacy) pada menu navigasi sidebar ([`config/navigation.ts`](file:///c:/Users/andym/Documents/Project%20Vue/tambora-frontend/config/navigation.ts)), meregistrasi skema form dengan pemilih koordinat peta OpenLayers interaktif (`coordinate-picker`) ([`schemas/master/organization.schema.ts`](file:///c:/Users/andym/Documents/Project%20Vue/tambora-frontend/schemas/master/organization.schema.ts)), serta mengintegrasikan pemanggilan API detail asinkron `GET /api/v1/organization/{id}` via `useAsyncDetail` dan standar SSOT `:record="detailRecord"` pada [`pages/home/master/organization.vue`](file:///c:/Users/andym/Documents/Project%20Vue/tambora-frontend/pages/home/master/organization.vue).
- **validasi-form-required-konsisten:** Standarisasi penanganan `required: true/false` pada modal form ([`components/base/BaseFormModal.vue`](file:///c:/Users/andym/Documents/Project%20Vue/tambora-frontend/components/base/BaseFormModal.vue)) dan perender form ([`components/base/FormFieldRenderer.vue`](file:///c:/Users/andym/Documents/Project%20Vue/tambora-frontend/components/base/FormFieldRenderer.vue)):
  - Menghubungkan validasi `isFormValid` pada `coordinate-picker` agar memeriksa `latKey` (latitude) dan `lngKey` (longitude) saat `required: true`, serta tidak memblokir saat `required: false` / opsional.
  - Menambahkan validasi array kosong (`[]`) untuk field bertipe multi-select saat ditandai `required: true`.
  - Menyederhanakan dan menyamakan indikator bintang merah (`*`) pada seluruh field dan sub-label Latitude & Longitude hanya muncul jika `field.required` bernilai `true`.
