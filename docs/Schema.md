...
- **Master Data Lainnya**:
  ...
  - `OrganizationDTO`: Struktur hierarki unit/induk/wilayah (_parent-child_), Latitude, Longitude, Alamat, Keterangan (`/api/v1/organization`).
  - `AssetItem` (`CreateAssetRequest` & `UpdateAssetRequest`): 22-field schema (`kode_mesin`, `nama_mesin`, `kode_spln`, `serial_number`, `power_plant_id`, `system_id`, `kondisi_mesin`, `kode_bahan_bakar`, `daya_terpasang`, `daya_mampu_netto`, `daya_mampu_pasok`, `kapasitas`, `jenis_tegangan`, `tegangan_hv`, `tegangan_lv`, `merk_mesin`, `tipe_mesin`, `merk_generator`, `nama_trafo`, `status_kepemilikan_mesin`, `status_kepemilikan_kwh`, `tahun_operasi`).
  - `MachineConditionDTO`: Inventaris status kesiapan dan kondisi mesin pembangkit.
